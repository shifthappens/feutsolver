...
De deployworkflow bouwt de lexiconcache met de interpreter van de service: eerst repository-variable `FEUTSOLVER_SERVICE_PYTHON`, daarna `<DEPLOY_PATH>/venv/bin/python` en anders de interpreter van het draaiende proces of de vaste `ExecStart` van `feutsolver.service`. Zet `FEUTSOLVER_SERVICE_USER` op de effectieve systemd-gebruiker als dat niet `www-data` is. De deploygebruiker moet die gebruiker zonder wachtwoord kunnen gebruiken voor de lock-schrijftest; zonder die controle wordt de deploy afgebroken.
