...
Opgeslagen spellen worden versie `v1` in de browseropslag (`localStorage`) bewaard. De eerste `Opslaan als…` vereist een niet-lege naam. Bestaat die naam al (hoofdletterongevoelig), dan vraagt de app eerst expliciet bevestiging voordat het bestaande spel wordt overschreven; bij annuleren blijft het ongewijzigd. Daarna werkt `Opslaan` het gekoppelde spel bij en maakt `Naam wijzigen` de naamswijziging direct blijvend. `Laden` herstelt de volledige stand. Corruptie of een onbekende opslagversie wordt overgeslagen met een waarschuwing. Handmatige bordwijzigingen en `Zet plaatsen` worden niet automatisch opgeslagen; gebruik `Opslaan` om een gekoppeld spel bewust bij te werken. Zonder bestaand spel maakt een wijziging niets aan. Het inladen van een schermafbeelding sluit de actieve spelopslag eerst, zodat de nieuwe stand niet per ongeluk in het oude spel wordt opgeslagen.

Een onderbroken sessie kost het bord niet meer. Streamlit bewaart de serversessie nu twee uur nadat de websocket wegvalt, bijvoorbeeld doordat een telefoon in slaap gaat of het tabblad naar de achtergrond verhuist. Verdwijnt die sessie alsnog — na een langere onderbreking, een herladen tabblad of een herstart van de service — dan herkent de browser de nieuwe, lege sessie aan een gewijzigde sessie-identificatie en stuurt hij het bord dat je ziet terug, in plaats van het te laten vervangen door een leeg standaardbord. De server valideert die stand opnieuw en berekent de suggesties met de eigen oplosser; suggesties, punten of kruiswoorden uit de browser worden nooit overgenomen. Bord, rek en een lopend voorbeeld blijven daardoor staan, ook als je pas na een pauze verdergaat. Het zichtbare bord wordt bovendien tijdens een voorbeeld in `sessionStorage` bewaard, zodat een herladen tabblad dezelfde stand terugkrijgt.

`Zet plaatsen` overleeft dat verlies eveneens. De klik stuurt de stand mee waarop de suggestie is berekend. Kent de server het bijbehorende oplosresultaat niet meer, dan wordt de aanvraag volledig opnieuw gecontroleerd: de meegestuurde stand moet bij zijn eigen hash horen en de oplosser moet exact die zet voor exact dat bord nog steeds aanbieden. Pas als dat niet lukt verschijnt de melding met foutcode `PLACE-STALE`, en ook dan blijft het bord staan. Een identiteitswissel is de uitzondering: de browseropslag krijgt dan een andere namespace en de serverstand blijft leidend, zodat het bord van een vorige gebruiker nooit terugkomt. Zonder betrouwbare identiteit krijgt iedere nieuwe sessie een eigen tijdelijke namespace; herstel over sessies heen werkt daar niet.
...
Naast iedere zichtbare suggestie staat een kruisje. Daarmee verwijder je die zet direct uit de beschikbare woorden: de app sluit het woord eerst alleen tijdens de herberekening uit, zodat nieuwe suggesties direct zichtbaar worden zonder dat woord. Daarna verwijdert één achtergrondactie het woord uit de woordenlijst en wordt de cache na een geslaagde mutatie direct ongeldig gemaakt; de deployment bouwt hem opnieuw op vóór de release actief wordt. De kruisjes blijven tijdens die update tijdelijk geblokkeerd. De woordenlijstschrijfacties gebruiken dezelfde bestandslock als de deploymerge, zodat een productie-aanpassing niet midden in een deploy verloren gaat.
...
De browser ontvangt alleen de gevalideerde bordstatus en de door de server berekende suggesties om ze te tonen. Iedere bordwijziging en iedere plaatsingsaanvraag wordt opnieuw server-side gevalideerd. Dat geldt ook voor een bord dat de browser na sessieverlies terugstuurt: die stand wordt als onbetrouwbare invoer gevalideerd en de zetten worden opnieuw berekend. De lokale opslag in de browser bevat alleen opgeslagen bordstanden, nooit OCR-code, solvercode of woordenlijstinhoud. `tests/test_client_server_boundary.py` bewaakt deze grens.
